...
The Agentic Office is a local two-process simulation: a Phaser 3 vintage office in the browser, and a Node harness that owns time, inventory, money, and the agents.
...
| P4 | The human player is the **lobby customer**, not an invisible operator. Sales outreach starts from the floor (Jim/Dwight), not from the player. |
...
| P10 | Human asks stay inside the paper business: no shells, no web, no jailbreaks, no token dumps. |

### 1.2 Non-goals (v1)

- Pathfinding around furniture
- Voice, 3D, or a custom-drawn map
- Cloud deploy (local preview only unless the operator asks)
...
| Tree | Owns |
| --- | --- |
| `src/game/` | Scenes, sprites, HUD, customer chat, camera. Interpolates events; does not simulate the business. |
...
| --- | --- | --- |
| Bind | `127.0.0.1` only | Local demo; keys never leave the machine via a public bind |
| Human line | ≤ 160 chars / 32 words / 80 estimated tokens | Stop prompt stuffing |
...
| Clock | 1 sim-minute per tick, Mon 09:00–17:00 | Real-time enough to play as customer |
...
- [x] Intro boots, Settings persist provider/goal/speed, Office loads the floorplan.
...
- [x] Customer chat is small, draggable, minimizable.
- [x] HUD shows clock, goal, books, Jira board, watercooler.

### 4.2 Office day

- [x] 09:00 LOG IN, 10:00 standup in conference, 12:00 lunch, 17:00 wrap — without a human click.
- [x] Morning straw draw; winner pitches from the desk.
- [x] “Yes” / “I’ll take them” rings up stock and cash; tickets on the board move.
...
- [x] `npm run eval` passes (22 cases: guardrails, commerce, goals, canned-reply cache).
...
| Goal | Win |
| --- | --- |
| Close one lobby sale | First successful ring-up |
...
| Player role | Lobby customer | Invisible HQ only | Makes the sale the game |
| Sales voices | Jim (dry) + Dwight (strict), straw draw for first pitch | Single salesperson | Coworking pair, visible conflict, still one chat lead |
...
**Client → harness:** `start`, `customer_say`, `human_reply`, `set_speed`.

**Harness → game:** `session_started`, `tick`/`clock`, `say`/`whisper`/`watercooler`/`dm`, `meeting_*`, `customer_line`, `ask_human`, `move_to`, `task_update`/`queue_update`, `books`/`stock`/`sale`/`reorder_*`, `bell`, `fire`, `goal_met`, `guardrail`, `error`.
